 ...
 */
package neuralnet.app.generate.generators.java;

import neuralnet.Encog;
import neuralnet.EncogError;
import neuralnet.app.generate.generators.AbstractGenerator;
import neuralnet.app.generate.program.EncogGenProgram;
import neuralnet.app.generate.program.EncogProgramNode;
import neuralnet.app.generate.program.EncogTreeNode;
import neuralnet.ml.MLFactory;
...
public class GenerateEncogJava extends AbstractGenerator {
	...
	private void embedNetwork(final EncogProgramNode node) {
		...
		// header
		addInclude("neuralnet.ml.MLMethod");
		addInclude("neuralnet.persist.EncogDirectoryPersistence");
		...
		// create factory
		line.setLength(0);
		addInclude("neuralnet.ml.factory.MLMethodFactory");
		...
		line.setLength(0);
		addInclude("neuralnet.ml.MLEncodable");
	...
	private void generateLoadTraining(final EncogProgramNode node) {
		...
		addInclude("neuralnet.ml.data.MLDataSet");
		final StringBuilder line = new StringBuilder();
		...
		if (this.embed) {
			addInclude("neuralnet.ml.data.basic.BasicMLDataSet");
			line.append("MLDataSet result = new BasicMLDataSet(INPUT_DATA,IDEAL_DATA);");
		} else {
			addInclude("neuralnet.util.simple.EncogUtility");
